...
Resend is used only from the server for security-sensitive transactional mail:

- account email verification;
- verification-email resend;
- password-reset instructions.

The raw verification/reset token is generated by the authentication service,
stored only as a one-way hash in PostgreSQL, and passed in memory to the email
provider. It is never returned by the public API and must never be logged.

Verification links target `/verify-email` and password-reset links target
`/reset-password`. The provider adapter makes a single non-retried POST for each
transactional message; repeating a non-idempotent email send automatically could
produce duplicate messages.

Development can boot without Resend credentials, but production startup fails
fast when `EMAIL_PROVIDER=resend` is not fully configured. CI unit/smoke tests use
an injected HTTP response and deliberately do not send a real email on every
push.

Before public launch, configure a verified sender/domain and re-check Resend's
current free/commercial limits, privacy terms and domain requirements.
...
Transactional email is not exposed as a public generic mail endpoint. It is
invoked only through authentication flows such as registration,
`/auth/resend-verification`, and `/auth/forgot-password`.

All public provider input is validated with shared Zod schemas before provider
calls.
...
- no automatic retry for transactional POST requests such as email delivery;
- provider-specific error mapping;
- response JSON validation boundaries;
- bounded TTL caches for weather, currency, geocoding, places, events, news and
  destination imagery;
- no caching of traveller-entered translation text or authentication emails.
...
features before every free API account has been created during development.

Production is stricter for core authentication email: startup requires the
Resend API key and sender identity so a public user cannot create an account and
then discover that verification/reset mail is impossible.
...
The planned no-cost development provider batch is now implemented:
