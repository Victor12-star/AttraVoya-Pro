...
test('resilience runner executes four soak waves, one spike, two probes, and bounded pauses', async () => {
  const calls = [];
  const pauses = [];
  const responses = [
    healthySoakWave(),
  ...
  assert.deepEqual(calls.slice(0, 4).map((call) => [call.requests, call.concurrency]), [
    [12, 4],
    [12, 4],
    [12, 4],
    [12, 4],
