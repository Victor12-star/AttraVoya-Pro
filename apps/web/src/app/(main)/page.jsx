...
const DESTINATIONS = [
  { city: 'Barcelona', country: 'ES', image: 'https://images.pexels.com/photos/1388030/pexels-photo-1388030.jpeg?auto=compress&cs=tinysrgb&w=1200' },
  { city: 'Kyoto', country: 'JP', image: 'https://images.pexels.com/photos/402028/pexels-photo-402028.jpeg?auto=compress&cs=tinysrgb&w=1200' },
  { city: 'Cape Town', country: 'ZA', image: 'https://images.pexels.com/photos/259447/pexels-photo-259447.jpeg?auto=compress&cs=tinysrgb&w=1200' },
];
    ...
    <>
      ...
      <section className="section">
        <div className="shell section-heading">
          ...
          <Link className="text-link" href="/explore">{messages.home.exploreCta} <ArrowUpRight size={16} /></Link>
        </div>
        <div className="shell destination-grid">
          {DESTINATIONS.map((destination) => (
            <Link className="destination-card" href={`/explore?destination=${encodeURIComponent(destination.city)}`} key={destination.city}>
              <span className="destination-card__image" style={{ backgroundImage: `url(${destination.image})` }} aria-hidden="true" />
              ...
              <span className="destination-card__action" aria-hidden="true"><ArrowUpRight size={18} /></span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="section section--editorial">
        <div className="shell editorial-grid">
          ...
          <div className="editorial-copy">
            <span className="eyebrow"><BedDouble size={15} /> {messages.navigation.stays}</span>
            ...
            <Link className="button button--dark" href="/stays">{messages.navigation.stays} <ArrowUpRight size={17} /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="shell section-heading">
          <div>
            <span className="eyebrow"><HeartHandshake size={15} /> {messages.budget.children}</span>
        ...
        <div className="shell family-band">
          ...
          <div className="family-band__content">
            <div className="family-band__numbers">
              <span>0–3</span><span>4–8</span><span>9–12</span><span>13–17</span>
            </div>
            <p>{messages.home.familyDescription}</p>
            <Link className="button button--light" href="/family">{messages.home.family} <ArrowUpRight size={17} /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="section section--tools">
        ...
        <div className="shell tool-grid">
          <Link href="/nearby"><MapPinned /><strong>{messages.navigation.nearby}</strong><ArrowUpRight /></Link>
          <Link href="/currency"><WalletCards /><strong>{messages.common.chooseCurrency}</strong><ArrowUpRight /></Link>
          <Link href="/language"><Languages /><strong>{messages.common.chooseLanguage}</strong><ArrowUpRight /></Link>
          <Link href="/things-to-do"><ShoppingBag /><strong>{messages.navigation.thingsToDo}</strong><ArrowUpRight /></Link>
          <Link href="/stays"><Building2 /><strong>{messages.navigation.stays}</strong><ArrowUpRight /></Link>
          <Link href="/transport"><TrainFront /><strong>{messages.home.local}</strong><ArrowUpRight /></Link>
        </div>
      </section>

      <section className="section section--safety">
        <div className="shell safety-panel">
          <div className="safety-panel__icon"><ShieldCheck size={30} /></div>
          <div>
          ...
          <Link className="button button--dark" href="/safety">{messages.safety.title} <ArrowUpRight size={17} /></Link>
        </div>
      </section>
    </>
  );
