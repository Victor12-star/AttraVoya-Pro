...
import styles from './hospitals-page.module.css';

const SEARCH_RADIUS_METERS = 20_000;
const SEARCH_LIMIT = 20;

/** @typedef {{status: 'idle'|'loading'|'success'|'empty'|'error', data: any}} HospitalState */
 ...
 */
function normalizeHospital(place, index, destinationCountryCode, resultProvider) {
 ...
 */
function normalizeHospitals(rows, countryCode, provider) {
  if (!Array.isArray(rows)) return [];

  const seen = new Set();
  const hospitals = [];
  rows.forEach((place, index) => {
    const hospital = normalizeHospital(place, index, countryCode, provider);
    if (!hospital || seen.has(hospital.key)) return;
    seen.add(hospital.key);
 ...
 * @param {string} locale
 * @returns {Promise<HospitalState>}
 */
async function requestHospitals(destination, locale) {
  try {
    const response = /** @type {any} */ (
      await apiClient.getNearbyPlaces({
        categoryGroup: PLACE_CATEGORY_GROUPS.HOSPITALS,
    ...
    const hospitals = normalizeHospitals(places?.results, destination.countryCode, provider);

    return {
      status: hospitals.length ? 'success' : 'empty',
      data: {
        provider,
...
export function HospitalsDestinationPage({ destination, locale = 'en', messages }) {
  const copy = getHospitalsPageCopy(locale);
  ...
  useEffect(() => {
    ...
    void requestHospitals(destination, locale).then((nextState) => {
  ...
  function retry() {
    ...
    void requestHospitals(destination, locale).then(setState);
  }

  if (!destination) {
    return (
      <section className={styles.page} aria-labelledby="hospitals-unavailable-title">
        <div className={`shell ${styles.stateShell}`}>
          <Hospital size={34} aria-hidden="true" />
          <h1 id="hospitals-unavailable-title">{messages.common.unavailable}</h1>
  ...
  return (
    <section className={styles.page} aria-labelledby="hospitals-title">
      <div className={`shell ${styles.shell}`}>
        ...
        <header className={styles.hero}>
          <div>
            ...
            <h1 id="hospitals-title">{destinationText(copy.title, destination.name)}</h1>
            <p>{copy.intro}</p>
          </div>
        ...
        {state.status === 'success' ? (
          <section aria-labelledby="hospitals-results-title">
            <h2 className={styles.resultsTitle} id="hospitals-results-title">
              {copy.results}
            </h2>
            <div className={styles.grid} aria-live="polite">
              {hospitals.map((hospital) => {
                const distance = formatDistance(numberFormatter, hospital.distanceMeters);
                return (
                  <article className={styles.card} key={hospital.key}>
                    ...
                    {hospital.formattedAddress ? (
                      <p className={styles.address}>
                        ...
                        <span>{hospital.formattedAddress}</span>
                      </p>
                    ) : null}
