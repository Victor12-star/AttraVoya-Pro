...
import styles from './police-page.module.css';

const SEARCH_RADIUS_METERS = 15_000;
const SEARCH_LIMIT = 20;

/** @typedef {{status: 'idle'|'loading'|'success'|'empty'|'error', data: any}} PoliceState */
 ...
 * Unexpected police-service claims remain deliberately ignored.
 * @param {any} place
 * @param {number} index
 * @param {string} destinationCountryCode
 * @param {string|null} resultProvider
 */
function normalizePolicePlace(place, index, destinationCountryCode, resultProvider) {
...
function normalizePolicePlaces(rows, countryCode, provider) {
  if (!Array.isArray(rows)) return [];

  const seen = new Set();
  const places = [];
  rows.forEach((place, index) => {
    const normalized = normalizePolicePlace(place, index, countryCode, provider);
...
/** @param {any} destination @param {string} locale @returns {Promise<PoliceState>} */
async function requestPolicePlaces(destination, locale) {
  try {
    const response = /** @type {any} */ (
      await apiClient.getNearbyPlaces({
        categoryGroup: PLACE_CATEGORY_GROUPS.POLICE,
...
export function PoliceDestinationPage({ destination, locale = 'en', messages }) {
  const copy = getPolicePageCopy(locale);
  ...
  useEffect(() => {
    ...
    void requestPolicePlaces(destination, locale).then((nextState) => {
  ...
  function retry() {
    ...
    void requestPolicePlaces(destination, locale).then(setState);
  }

  if (!destination) {
    return (
      <section className={styles.page} aria-labelledby="police-unavailable-title">
        <div className={`shell ${styles.stateShell}`}>
          <Shield size={34} aria-hidden="true" />
          <h1 id="police-unavailable-title">{messages.common.unavailable}</h1>
  ...
  const policePlaces = state.data?.policePlaces ?? [];
  ...
  return (
    <section className={styles.page} aria-labelledby="police-title">
      <div className={`shell ${styles.shell}`}>
        ...
        <header className={styles.hero}>
          <div>
            ...
            <h1 id="police-title">{destinationText(copy.title, destination.name)}</h1>
            <p>{copy.intro}</p>
          </div>
        ...
        {state.status === 'success' ? (
          <section aria-labelledby="police-results-title">
            <h2 className={styles.resultsTitle} id="police-results-title">
              {copy.results}
            </h2>
            <div className={styles.grid} aria-live="polite">
              {policePlaces.map((place) => {
