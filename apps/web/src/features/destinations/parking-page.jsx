...
import styles from './parking-page.module.css';

const SEARCH_RADIUS_METERS = 10_000;
const SEARCH_LIMIT = 24;

/** @typedef {{status: 'idle'|'loading'|'success'|'empty'|'error', data: any}} ParkingState */
 ...
 * Unexpected availability, price, restriction, payment, reservation and access claims stay ignored.
 * @param {any} place
 * @param {number} index
 * @param {string} destinationCountryCode
 * @param {string|null} resultProvider
 */
function normalizeParkingPlace(place, index, destinationCountryCode, resultProvider) {
...
function normalizeParkingPlaces(rows, countryCode, provider) {
  if (!Array.isArray(rows)) return [];

  const seen = new Set();
  const parkingPlaces = [];
  rows.forEach((place, index) => {
    const normalized = normalizeParkingPlace(place, index, countryCode, provider);
    if (!normalized || seen.has(normalized.key)) return;
    seen.add(normalized.key);
    parkingPlaces.push(normalized);
  });

  return parkingPlaces.sort((left, right) => {
...
/** @param {any} destination @param {string} locale @returns {Promise<ParkingState>} */
async function requestParking(destination, locale) {
  try {
    const response = /** @type {any} */ (
      await apiClient.getNearbyPlaces({
        categoryGroup: PLACE_CATEGORY_GROUPS.PARKING,
    ...
    const parkingPlaces = normalizeParkingPlaces(
      places?.results,
    ...
    return {
      status: parkingPlaces.length ? 'success' : 'empty',
      data: {
        provider,
...
export function ParkingDestinationPage({ destination, locale = 'en', messages }) {
  const copy = getParkingPageCopy(locale);
  ...
  useEffect(() => {
    ...
    void requestParking(destination, locale).then((nextState) => {
  ...
  function retry() {
    ...
    void requestParking(destination, locale).then(setState);
  }

  if (!destination) {
    return (
      <section className={styles.page} aria-labelledby="parking-unavailable-title">
        <div className={`shell ${styles.stateShell}`}>
          <CircleParking size={34} aria-hidden="true" />
          <h1 id="parking-unavailable-title">{messages.common.unavailable}</h1>
  ...
  const parkingPlaces = state.data?.parkingPlaces ?? [];
  ...
  return (
    <section className={styles.page} aria-labelledby="parking-title">
      <div className={`shell ${styles.shell}`}>
        ...
        <header className={styles.hero}>
          <div>
            ...
            <h1 id="parking-title">{destinationText(copy.title, destination.name)}</h1>
            <p>{copy.intro}</p>
          </div>
        ...
        {state.status === 'error' ? (
          <div className={styles.feedback} role="alert">
            <CircleParking size={28} aria-hidden="true" />
        ...
        {state.status === 'success' ? (
          <section aria-labelledby="parking-results-title">
            <h2 className={styles.resultsTitle} id="parking-results-title">
              {copy.results}
            </h2>
