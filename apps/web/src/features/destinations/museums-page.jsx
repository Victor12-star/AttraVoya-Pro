...
import styles from './museums-page.module.css';

const SEARCH_RADIUS_METERS = 15_000;
const SEARCH_LIMIT = 24;

/** @typedef {{status: 'idle'|'loading'|'success'|'empty'|'error', data: any}} MuseumState */
 ...
 */
function normalizeMuseum(place, index, destinationCountryCode, resultProvider) {
 ...
 */
function normalizeMuseums(rows, countryCode, provider) {
  if (!Array.isArray(rows)) return [];

  const seen = new Set();
  const museums = [];
  rows.forEach((place, index) => {
    const museum = normalizeMuseum(place, index, countryCode, provider);
    if (!museum || seen.has(museum.key)) return;
    seen.add(museum.key);
    museums.push(museum);
  });

  return museums.sort((left, right) => {
 ...
 * @param {string} locale
 * @returns {Promise<MuseumState>}
 */
async function requestMuseums(destination, locale) {
  try {
    const response = /** @type {any} */ (
      await apiClient.getNearbyPlaces({
        categoryGroup: PLACE_CATEGORY_GROUPS.MUSEUMS,
    ...
    const museums = normalizeMuseums(places?.results, destination.countryCode, provider);

    return {
      status: museums.length ? 'success' : 'empty',
      data: {
        provider,
...
export function MuseumsDestinationPage({ destination, locale = 'en', messages }) {
  const copy = getMuseumsPageCopy(locale);
  ...
  useEffect(() => {
    ...
    void requestMuseums(destination, locale).then((nextState) => {
  ...
  function retry() {
    ...
    void requestMuseums(destination, locale).then(setState);
  }

  if (!destination) {
    return (
      <section className={styles.page} aria-labelledby="museums-unavailable-title">
        <div className={`shell ${styles.stateShell}`}>
          <Landmark size={34} aria-hidden="true" />
          <h1 id="museums-unavailable-title">{messages.common.unavailable}</h1>
  ...
  const museums = state.data?.museums ?? [];
  ...
  return (
    <section className={styles.page} aria-labelledby="museums-title">
      <div className={`shell ${styles.shell}`}>
        ...
        <header className={styles.hero}>
          <div>
            ...
            <h1 id="museums-title">{destinationText(copy.title, destination.name)}</h1>
            <p>{copy.intro}</p>
          </div>
        ...
        {state.status === 'success' ? (
          <section aria-labelledby="museums-results-title">
            <h2 className={styles.resultsTitle} id="museums-results-title">
              {copy.results}
            </h2>
            <div className={styles.grid} aria-live="polite">
              {museums.map((museum) => {
                const distance = formatDistance(numberFormatter, museum.distanceMeters);
                return (
                  <article className={styles.card} key={museum.key}>
                    ...
                    {museum.formattedAddress ? (
                      <p className={styles.address}>
                        <MapPin size={16} aria-hidden="true" />
                        <span>{museum.formattedAddress}</span>
                      </p>
                    ) : null}
