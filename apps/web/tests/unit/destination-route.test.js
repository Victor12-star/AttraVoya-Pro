...
describe('destination route contract', () => {
  ...
  it('preserves destination context for child and main feature entry points', () => {
    ...
    expect(buildDestinationChildHref(stockholm, 'atms')).toContain(
      '/destinations/stockholm-se/atms?',
    );
    expect(buildDestinationChildHref(stockholm, 'parking')).toContain(
      '/destinations/stockholm-se/parking?',
    );
    expect(buildDestinationChildHref(stockholm, 'cafes')).toContain(
      '/destinations/stockholm-se/cafes?',
    );
