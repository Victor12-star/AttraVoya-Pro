...
describe('Frankfurter adapter', () => {
  it('converts with a real provider rate shape and marks values approximate', async () => {
    const http = { requestJson: vi.fn().mockResolvedValue({ date: '2026-09-03', base: 'SEK', quote: 'EUR', rate: 0.09 }) };
    ...
    expect(result).toMatchObject({ from: 'SEK', to: 'EUR', rate: 0.09, convertedAmount: 90, approximate: true });
  });
});
