...
export function normalizeFrankfurterRates(payload, expectedBase) {
  if (Array.isArray(payload)) {
    return payload.map(normalizeRateRow).filter((row) => row.base && row.quote && Number.isFinite(row.rate));
  }
...
export function normalizeFrankfurterSingleRate(payload, base, quote) {
  ...
  const normalized = normalizeRateRow({ ...candidate, base: candidate?.base ?? base, quote: candidate?.quote ?? quote });
