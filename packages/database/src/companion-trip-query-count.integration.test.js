...
describeCompanionTripQueryCount(
  'Travel Companion PostgreSQL query-count contract',
  () => {
    it(
      'keeps destination relation SQL bounded from one trip through the full companion limit',
      async () => {
        const connectionString = process.env.DATABASE_URL;
        if (!connectionString) {
          throw new Error(
            'DATABASE_URL is required for the companion trip query-count test.',
          );
        }
        ...
        try {
          ...
          async function measure(limit) {
            ...
            const rows = await repository.listOwnedCompanionTrips({
              userId,
              today,
              limit,
            });
