...
const languageCode = z.string().trim().min(2).max(10).regex(/^[A-Za-z-]+$/);
...
export const currencyRatesQuerySchema = z.object({
  ...
  quotes: z.string().trim().max(100).optional().transform((value) => value
...
export const translationBodySchema = z.object({
  ...
  target: languageCode,
}).strict().refine((value) => value.source === 'auto' || value.source !== value.target, {
...
export const accommodationNearbyQuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(50).default(20),
  language: languageCode.default('en'),
  types: z.string().trim().max(300).optional().transform((value) => value
    ? value.split(',').map((item) => item.trim().toUpperCase()).filter(Boolean)
    : []),
}).strict().superRefine((value, context) => {
